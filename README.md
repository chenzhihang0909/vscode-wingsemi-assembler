...
[Wingsemi Assembler](https://github.com/wingsemi-assembler/wingsemi-assembler) is a popular online C++ compiler that supports various platforms and different compiler versions. Not only can you view the execution results, but you can also inspect the assembly code generated by the compiler. This plugin integrates Wingsemi Assembler into VSCode based on the [API](https://github.com/wingsemi-assembler/wingsemi-assembler/blob/main/docs/API.md) provided by Wingsemi Assembler, allowing users to directly utilize Wingsemi Assembler's features within VSCode.

## Installation

The extension is available on the [VS Code Marketplace](https://marketplace.visualstudio.com/). Search the marketplace for an extensonion named "Wingsemi Assembler for VSCode" or just follow [this link](https://marketplace.visualstudio.com/items?itemName=ykiko.vscode-wingsemi-assembler).
...
## User Settings
`wingsemi-assembler.default.options`: Default parameters when creating a compiler using the `+` button.
```json
...
`wingsemi-assembler.default.color`：Colors used for highlighting assembly code.
...
```
`wingsemi-assembler.default.url`：Default link loaded when opening the plugin, default is empty.
...
## Issue Feedback
This plugin is still in its early stages. If you encounter any issues during usage or have any suggestions, feel free to discuss them on [GitHub](https://github.com/16bit-ykiko/vscode-wingsemi-assembler). Additionally, the Output window may provide some useful information.
